...
if [[ "$USE_RAMDISK" ]]; then
  ...
  if [[ "$IS_BROADCAST_NODE" ]]; then
    aws s3 cp s3://$S3_BUCKET/broadcast-$VERSION-latest.tar.lz4 .
    cat broadcast-$VERSION-latest.tar.lz4 | lz4 -d | tar x --wildcards 'blockchain/block*' -C /mnt/ramdisk 'blockchain/shared*'
  elif [[ "$IS_AH_NODE" ]]; then
    aws s3 cp s3://$S3_BUCKET/ahnode-$VERSION-latest.tar.lz4 .
    cat ahnode-$VERSION-latest.tar.lz4 | lz4 -d | tar x --wildcards 'blockchain/block*' -C /mnt/ramdisk 'blockchain/shared*'
  else
    aws s3 cp s3://$S3_BUCKET/blockchain-$VERSION-latest.tar.lz4 .
    cat blockchain-$VERSION-latest.tar.lz4 | lz4 -d | tar x --wildcards 'blockchain/block*' -C /mnt/ramdisk 'blockchain/shared*'
  fi
  chown -R steemd:steemd /mnt/ramdisk/blockchain
else
  if [[ "$IS_BROADCAST_NODE" ]]; then
    aws s3 cp s3://$S3_BUCKET/broadcast-$VERSION-latest.tar.lz4 .
    cat broadcast-$VERSION-latest.tar.lz4 | lz4 -d | tar x
  elif [[ "$IS_AH_NODE" ]]; then
    aws s3 cp s3://$S3_BUCKET/ahnode-$VERSION-latest.tar.lz4 .
    cat ahnode-$VERSION-latest.tar.lz4 | lz4 -d | tar x
  else
    aws s3 cp s3://$S3_BUCKET/blockchain-$VERSION-latest.tar.lz4 .
    cat blockchain-$VERSION-latest.tar.lz4 | lz4 -d | tar x
  fi
fi
if [[ $? -ne 0 ]]; then
  if [[ ! "$SYNC_TO_S3" ]]; then
